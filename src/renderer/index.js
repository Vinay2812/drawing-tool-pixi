...
export const renderFigmaJson = async (figmaJson, elementId) => {
	...
	PIXI.Assets.load(imageUrls).then(() => {
		...
		if (!canvas) {
			...
			const screenWidth =
				window.innerWidth > 400 ? 400 * resolutionFactor : window.innerWidth * resolutionFactor;
			...
			const scaleWidth =
				screenWidth / figmaJson?.absoluteBoundingBox?.width;
			const scaleHeight =
				screenHeight / figmaJson?.absoluteBoundingBox?.height;
